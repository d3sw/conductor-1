...
public class AuroraErrorLookupDAO extends AuroraBaseDAO implements ErrorLookupDAO {
	private static final Logger logger = LoggerFactory.getLogger(AuroraErrorLookupDAO.class);
	...
	public List<ErrorLookup> getErrors() {
		...
		return getWithTransaction(tx -> query(tx, SQL, q -> q.executeAndFetch(handler)));
	}
	...
	public List<ErrorLookup> getErrorMatching(String workflow, String errorString) {
		...
		return getWithTransaction(tx -> query(tx, SQL.toString(), q -> q.
			addParameter(errorString).
			addParameter(workflow).
	...
	public List<ErrorLookup> getErrorByCode(String errorCode) {
		...
		return getWithTransaction(tx -> query(tx, SQL, q -> q.
	...
	static class ErrorLookupHandler implements ResultSetHandler<ArrayList<ErrorLookup>> {
